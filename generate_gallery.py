...
- Each gallery includes:
  ...
  - Keyboard navigation (Left/Right) and 'I' to open current file in file explorer (provides helper .bat/.sh to run locally)
"""
import os
import sys
import json
...
<style>
  ...
  .card.selected{outline:3px solid #2b8ae2}
...
(function(){
  ...
  function selectIndex(i){
    if(i<0) i = FILE_LIST.length-1;
    if(i>=FILE_LIST.length) i = 0;
    state.currentIndex = i;
    const children = Array.from(grid.children);
    children.forEach(c=>c.classList.toggle('selected', c.dataset.rel === FILE_LIST[i]));
    // ensure visible
    const el = children.find(c=>c.dataset.rel === FILE_LIST[i]);
    if(el) el.scrollIntoView({behavior:'smooth',block:'center'});
  }

  function renderGrid(){
    grid.innerHTML = '';
    FILE_LIST.forEach(function(rel, idx){
    ...
    if(FILE_LIST.length>0){
      if(state.currentIndex >= FILE_LIST.length) state.currentIndex = 0;
  ...
  function buildBashScript(){
    const lines = ['#!/bin/sh','# Delete script generated by media_watcher','', 'cd "' + MEDIA_ROOT + '" || exit 1', ''];
    Array.from(state.marked).forEach(rel=>{
      lines.push('rm -v -- "' + rel.replace(/"/g,'\"') + '"');
    });
    return lines.join('\n') + '\n';
  }

  function buildWinScript(){
    const lines = ['@echo off','rem Delete script generated by media_watcher','', 'pushd "' + MEDIA_ROOT + '" || exit /b 1', ''];
    Array.from(state.marked).forEach(rel=>{
      const escaped = rel.replace(/"/g,'""');
      lines.push('del /f /q "' + escaped + '"');
    });
    lines.push('popd');
    return lines.join('\r\n') + '\r\n';
  }

  function buildExplorerHelpers(rel){
    // provide shell helpers user can run locally
    const full = (MEDIA_ROOT.replace(/\\/+$/,'') + '/' + rel).replace(/\\/g,'/');
    const bash = '#!/bin/sh\n# Open containing folder\nxdg-open "' + full.replace(/"/g,'\"') + '"\n';
    const bat = 'explorer.exe /select,"' + full.replace(/"/g,'"') + '"\r\n';
    return {bash: bash, bat: bat};
  }
  ...
  function openInExplorer(rel){
    // Try to open folder in browser via file:// (best-effort) and also offer a helper .bat to run that will select the file in Explorer.
    const fullPath = (MEDIA_ROOT.replace(/\\/+$/,'') + '/' + rel).replace(/\\/g,'/');
    // open containing folder in browser
    const folder = fullPath.split('/').slice(0,-1).join('/') || fullPath;
    try{ window.open('file://' + folder, '_blank'); } catch(e){}
    // offer helper download
    const helpers = buildExplorerHelpers(rel);
    if(confirm('Browsers cannot always launch the system file explorer. Download a small helper (.bat) to open Explorer and select the file?')){
      download('open_select.bat', helpers.bat);
      alert('Downloaded open_select.bat - run it to open Explorer and select the file.');
    }
  }

  // keyboard handling: left/right to change selection, I to open in explorer, Enter to view
  document.addEventListener('keydown', function(e){
    ...
    if(e.key === 'ArrowLeft'){
      ...
      e.preventDefault();
      const rel = FILE_LIST[state.currentIndex];
      openInExplorer(rel);
    } else if(e.key === 'Enter'){
      e.preventDefault();
      const rel = FILE_LIST[state.currentIndex];
      ...
      window.open(fullPath, '_blank');
      if(autoDeleteEl.checked){
        const card = Array.from(grid.children).find(c=>c.dataset.rel === rel);
        if(card) performDelete(rel, card);
      }
    }
  });
...
def generate_for_folder(folder: Path):
    ...
    # use absolute media folder path so generated delete scripts work from any cwd
...
def main():
    ...
    if generated:
        ...
        print("Notes:")
        print(" - The generated delete scripts (downloadable from the gallery UI) cd into the media folder so you can run them from any cwd.")
        print(" - The browser cannot directly open your system file explorer; the gallery offers a small .bat helper you can download to open Explorer and select a file.")
    else:
